...
# ===========================
# BALLENA Processing Pipeline
# ===========================

# This script is the heart of the BALLENA processing pipeline, it handles all attributes except for name and correctly 
# parses all the entries into csv file, processes them with the appropriate finetuned models and hits@k values, 
# then it performs the similarity search and evaluates the results. It can be customized to only do certain attributes 
# or splits, and it has a testing mode for only testing its functionality on x number of rows.
...
def load_faiss_index(attribute, embeddings, index_directory='faiss_index'):
    """
...
def evaluate_all_attributes(attributes_info, output_dir='extraction_results/Nougat_FT', evaluation_output_dir='evaluation_results/Nougat_ft_evaluation_results', splits=['1st']):
    """
...
def extract_gpt_data(attributes_info, attributes_k, extracted_text_df, output_dir='extraction_results/Nougat_FT', test_mode=False, test_size=20, split='2nd'):
    """
