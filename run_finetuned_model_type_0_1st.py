...
# Define the collection type attribute information
...
# Processing the 'collectionType' attribute
attribute = 'collectionType'
info = collection_type_info
print(f"Processing attribute: {attribute}")

# Define the specific fine-tuned model name for 'collectionType' attribute
model_name = 'ft:gpt-4o-2024-08-06:eccenca-gmbh:ballena-type-0-1st:AEaere6p'
...
# Define the backup file for 'collectionType' attribute
...
with open(backup_path, 'a', newline='', encoding='utf-8') as backup_f:
    ...
    for _, row in tqdm(merged_df.iterrows(), total=merged_df.shape[0], desc=f"Attribute: {attribute}"):
        ...
        try:
            ...
            restored_values_list = [item.get('isolationType', '') for item in restored_data]
...
# Define the main output filename for 'collectionType' attribute
...
print("Processing complete for the 'collectionType' attribute.")